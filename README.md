...
### Вариант - Форматирование научной нотации в число с плавающей точкой на языке Python
### Примеры допустимых строк (верные строки):
```Python
stringformat = "{:f}".format(3.14e+4);
```
![Alt-текст](https://github.com/Eralandin/ImagesForKursach/blob/main/изображение_2025-05-11_205829557.png)

```Python
ctrutoformat = "{:f}".format(3.14e-4);
```
![Alt-текст](https://github.com/Eralandin/ImagesForKursach/blob/main/изображение_2025-05-11_205843914.png)

## Примеры допустимых строк (строки с ошибками):
```Python
str@ingformat = "{:f}".(3.144);
```
![Alt-текст](https://github.com/Eralandin/ImagesForKursach/blob/main/изображение_2025-05-11_210105700.png)

```Python
ctruto#format = ":f}".forat(3.14e-4);
```
![Alt-текст](https://github.com/Eralandin/ImagesForKursach/blob/main/изображение_2025-05-11_210031931.png)


# Включение семантики в анализатор. Создание внутренней формы представления программы.
### Постановка задачи - Дополнить анализатор, разработанный в рамках лабораторных работ, этапом формирования внутренней формы представления программы.
### Вариант задания - 
![Alt-текст](https://github.com/Eralandin/ImagesForKursach/blob/main/изображение_2025-05-11_210552086.png)

### Примеры верных строк:
```C#
2+3*(2+3)*((2*2)+1)
```
![Alt-текст](https://github.com/Eralandin/ImagesForKursach/blob/main/изображение_2025-05-11_210817932.png)

```C#
(34/2)*2 + 3
```
![Alt-текст](https://github.com/Eralandin/ImagesForKursach/blob/main/изображение_2025-05-11_210858869.png)

# Реализация алгоритма поиска подстрок с помощью регулярных выражений.
### Поставнока задачи - Реализовать алгоритм поиска в тексте подстрок, соответствующих заданным регулярным выражениям. Используя библиотечные реализации (например, класс Regex в C#), решить 3 задачи в соответствии с персональным вариантом (мои варианты - №25 для блока №1, №20 для блока №2 и №25 для блока №3)
...
### Решение трёх задач:
1. КПП (Код причины постановки на учёт) — 9-значное число: первые 4 цифры — ИНН, потом 2 — причина постановки, последние 3 — код налогового органа. <br>
Решение:<br>
Regex:
```C#
\b\d{4}\d{2}\d{3}\b
```
3. Размеры файлов состоят из числа (с десятичной точкой или без нее), за которым следуют KB, MB, GB или TB (необязательно ещё может быть пробел).<br>
Решение:<br>
Regex:
```C#
\b\d+(?:\.\d+)?\s?(?:KB|MB|GB|TB)\b
```
4. Формат: две буквы (не D, F, I, Q, U, V), 6 цифр, буква A-D или пусто.<br>
Решение:<br>
Regex:
```C#
\b(?!BG|GB|KN|NK|NT|TN|ZZ)(?!.*[DFIQUV])[A-CEGHJ-NPR-TW-Z]{2}\d{6}[A-D]?\b
```
### Тестовые примеры:

1. ![Alt-текст](https://github.com/Eralandin/ImagesForKursach/blob/main/изображение_2025-05-12_143711719.png)
2. ![Alt-текст](https://github.com/Eralandin/ImagesForKursach/blob/main/изображение_2025-05-12_143719101.png)
3. ![Alt-текст](https://github.com/Eralandin/ImagesForKursach/blob/main/изображение_2025-05-12_143726830.png)
...
Вариант №7:
Для грамматики G[<expr>] разработать и реализовать алгоритм анализа на основе метода рекурсивного спуска.
```C++
G[<expr>]:
1. <expr> ::= <expr_add_sub> | <expr><operator_mul_div> <expr_add_sub> 
2. <expr_add_sub> ::= <term> | <expr_add_sub><operator_mul_div> <term>
3. <term> ::= <integer> | "(" <expr> ")" 
4. <operator_mul_div> ::= "*" | "/" | "%"
5. <operator_add_sub> ::= "+" | "-"
...
### Язык
Грамматика описывает арифметические выражения, включающие:
1. Целые числа
2. Скобки для группировки
3. Знаки сложения и вычитания
4. Знаки умножения, деления, остатка от деления<br>
Т.е. L(G) - это множество строк, представляющих арифметически корректные выражения из целых чисел и операторов (+,-,*,/,%).

### Классификация грамматики:
При анализе правил вывода данной грамматики можно сделать вывод, что данная грамматика - контекстно-свободная, т.к. все правила имеют вид A -> α, где A — нетерминал, α — строка из терминалов и/или нетерминалов.
